/* ! Remark (http://getbootstrapadmin.com/remark) Copyright 2016 amazingsurge Licensed under the Themeforest Standard Licenses */ ! function(window, document) {
	"use strict";

	function getLevel(url, tag) {
		for(var level, arr = url.split("/").reverse(), path = "", i = 0; i < arr.length; i += 1) {
			arr[i] === tag && (level = i)
		}
		for(var m = 1; level > m; m += 1) {
			path += "../"
		}
		return path
	}
	...
	if(settings) {
		if("{" === settings[0] && (settings = JSON.parse(settings)), settings.primary && "primary" !== settings.primary) {
			...
			link.type = "text/css", link.rel = "stylesheet", link.href = getLevel(window.location.pathname, layout) + "../base/assets/skins/" + settings.primary + ".css", link.id = "skinStyle", head.appendChild(link)
		}
		if(settings.sidebar && "dark" === settings.sidebar) {
	...
	document.addEventListener && document.addEventListener("DOMContentLoaded", function() {
		var $body = $(document.body),
			...
			Skintools = {
				tpl: '<div class="site-skintools"><div class="site-skintools-inner"><div class="site-skintools-toggle"><i class="icon icon-pifu primary-600"></i></div><div class="site-skintools-content"><div class="nav-tabs-horizontal"><ul role="tablist" data-plugin="nav-tabs" class="nav nav-tabs nav-tabs-line"><li role="presentation" class="active"><a role="tab" aria-controls="skintoolsSidebar" href="#skintoolsSidebar" data-toggle="tab" aria-expanded="true">菜单</a></li><li role="presentation"><a role="tab" aria-controls="skintoolsNavbar" href="#skintoolsNavbar" data-toggle="tab" aria-expanded="false">导航</a></li><li role="presentation"><a role="tab" aria-controls="skintoolsPrimary" href="#skintoolsPrimary" data-toggle="tab" aria-expanded="false">全局</a></li></ul><div class="tab-content"><div role="tabpanel" id="skintoolsSidebar" class="tab-pane active"></div><div role="tabpanel" id="skintoolsNavbar" class="tab-pane"></div><div role="tabpanel" id="skintoolsPrimary" class="tab-pane"></div><button class="btn btn-block btn-primary margin-top-20" id="skintoolsReset" type="button">Reset</button></div></div></div></div></div>',
				...
				init: function() {
					var self = this;
					this.path = getLevel(window.location.pathname, layout), this.overflow = !1, this.$siteSidebar = $(".site-menubar"), this.$siteNavbar = $(".site-navbar"), this.$container = $(this.tpl), this.$toggle = $(".site-skintools-toggle", this.$container), this.$content = $(".site-skintools-content", this.$container), this.$tabContent = $(".tab-content", this.$container), this.$sidebar = $("#skintoolsSidebar", this.$content), this.$navbar = $("#skintoolsNavbar", this.$content), this.$primary = $("#skintoolsPrimary", this.$content), this.build(this.$sidebar, this.skintoolsSidebar, "skintoolsSidebar", "radio", "菜单皮肤"), this.build(this.$navbar, ["inverse"], "skintoolsNavbar", "checkbox", "导航属性"), this.build(this.$navbar, this.skintoolsNavbar, "skintoolsNavbar", "radio", "导航皮肤"), this.build(this.$primary, this.skintoolsPrimary, "skintoolsPrimary", "radio", "全局皮肤"), this.$container.appendTo($body), this.$toggle.on("click", function() {
				...
				primaryImprove: function(val) {
					var $link = $("#skinStyle", $("head")),
						href = this.path + "../base/assets/skins/" + val + ".css";
